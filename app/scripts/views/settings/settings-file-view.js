...
const SettingsFileView = Backbone.View.extend({
            ...
            if(storage.name === "ethereum"){
              if(!storage.is_curr_contract_address_valide()){
                        ...
                        if (storage.getOpenConfig) {
                            ...
                            const openConfigView = new OpenConfigEthView({ model: config });
                            Alerts.alert({
                                header: '',
                                body: '',
                                icon: storage.icon || 'files-o',
                                buttons: [Alerts.buttons.create,Alerts.buttons.cancel,Alerts.buttons.ok],
                                esc: '',
                                opaque: true,
                                view: openConfigView,
                                success: () => {
                                    ...
                                    if (storage.applyConfig) {
                                        storage.applyConfig(opts,this.storageApplyConfigComplete.bind(this,openConfigView));
                                    }
                                },
                                create: () => {
    ...
    storageApplyConfigComplete: function(openConfigView,err) {
        if (err) {
          Alerts.alert({
            header: err,
