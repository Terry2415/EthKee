...
const OpenConfigEthView = require('./open-config-eth-view');
...
const OpenView = Backbone.View.extend({
    ...
    showConfig: function(storage) {
        ...
        if(storage.name === "ethereum"){
            this.views.openConfig = new OpenConfigEthView({ el: this.$el.find('.open__config-wrap'), model: config }).render();
            this.views.openConfig.on('create', this.createConfig.bind(this));
        }
        else {
    ...
    createConfig: function(config) {
      ...
      if (storage.createConfig) {
          storage.createConfig(opts, this.storageCreateConfigComplete.bind(this, req));
      } else {
          storage.stat(path, opts, this.storageStatComplete.bind(this, req));
      }
    },

    storageCreateConfigComplete: function(req, err) {
    ...
    storageApplyConfigComplete: function(req, err) {
        ...
        if (err) {
            ...
            this.closeConfig();
            if(req.storage === "ethereum"){
              var storage = Storage[req.storage];
              if(storage.is_curr_contract_address_valide()){
                this.listStorage(storage);
              }
            }
        }
